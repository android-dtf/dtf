...
#
"""Integration tests for the "prop test" utility"""
...
def test_no_args():
    ...
    """Running test with no args"""

    testutils.deploy_config(testutils.get_default_config())

    rtn = testutils.dtf("prop test")
...
def test_test_valid_prop():

    """Test a valid property"""

    testutils.deploy_config(testutils.get_default_config())

    rtn = testutils.dtf("prop test Info sdk")

    testutils.undeploy()

    assert(rtn.return_code == 1)
